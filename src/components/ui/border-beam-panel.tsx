...
/* Border Beam Panel — from Motiq (https://motiq.dev/components/border-beam-panel).
   ...
   Re-themed for GENZ-WEARS: tokens below map to this project's own dark
   palette (--green / --violet / --bg / --panel-border in src/styles/index.css)
   instead of Motiq's default blue/coral, and `cn` is the project's shared
   src/lib/utils helper instead of a locally-duplicated clsx+tailwind-merge. */

/* -------------------------------------------------------------------------- */
/* Motiq design tokens — re-themed to the GENZ-WEARS palette                  */
/* -------------------------------------------------------------------------- */
/* Rendered with the component, in a low cascade layer, so your own
   `:root { --motiq-*: … }` always wins. Move it to globals.css to drop it. */
const MOTIQ_TOKENS =
  '@layer motiq{:root{--motiq-accent:#95ff8a;--motiq-accent-text:#6fe07f;--motiq-bg:#07090a;--motiq-border:#232823;--motiq-border-strong:#3a423c;--motiq-fg:#f3f5f2;--motiq-fg-secondary:#c7cec9;--motiq-muted:#8b948d;--motiq-secondary-accent:#8b6cff;--motiq-signature:#8b6cff;--motiq-surface:#0f1512;--motiq-surface-2:#121815}}';
...
function ringGradient(beams: 1 | 2, colors: [string, string?] | undefined): string {
  const tail0 = colors?.[0] ?? "var(--motiq-accent, #95ff8a)";
  // Default comet reads green → violet along its length; a custom color stays itself.
  const head0 = colors?.[0] ?? "var(--motiq-secondary-accent, #8b6cff)";
  const stops = [
    "transparent 0deg",
    comet(tail0, head0, `color-mix(in srgb, ${head0} 22%, #ffffff)`, 55, 0),
  ];
  // The single violet moment on this surface — the second comet's head.
  if (beams === 2) {
    const c1 = colors?.[1] ?? "var(--motiq-signature, #8b6cff)";
...
@media (forced-colors: active) {
  ...
  return (
    <div
      ...
      onBlur={settle}
      className={cn(
        "relative w-full border border-[var(--motiq-border,#232823)] bg-[var(--motiq-surface,#0f1512)] p-7",
        cls,
        className,
      )}
      style={{
    ...
    >
      ...
      {children}
      {/* Ring renders after children on purpose: both are position:relative/absolute
          with z-index:auto, and same-stacking-level positioned elements paint in DOM
          order. With no padding gap (a panel with opaque, edge-to-edge content —
          e.g. a product photo), a ring painted *before* children would be hidden
          underneath them. Rendering it last keeps the ring visible in that case
          without needing an explicit z-index, and doesn't change anything for the
          padded/transparent-content case the demo uses. */}
